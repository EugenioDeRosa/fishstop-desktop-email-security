...
- Può interrogare AlienVault OTX durante l'analisi usando soltanto gli indicatori tecnici presenti nell'email. Sono accettate esclusivamente corrispondenze native ed esatte: URL e hash richiedono un tag malevolo esplicito, i domini il tag `phishing`, mentre gli IP devono essere corroborati da AbuseIPDB. Non vengono scaricati feed o database locali.
...
Gli accessi Google e Microsoft usano OAuth Authorization Code con PKCE e callback loopback locale. Microsoft usa un client desktop pubblico e non richiede un client secret; nel portale Entra deve essere registrato `http://localhost` come URI di reindirizzamento per applicazioni mobili e desktop. Non vengono salvate password. I token temporanei del login non vengono conservati; quando l'utente collega volontariamente la casella, il solo refresh token necessario a mantenere l'accesso in lettura viene custodito nel portachiavi di sistema e rimosso da FishStop con `Disconnect`. Anche le chiavi API di reputazione sono conservate nel portachiavi; cronologia e preferenze restano sul dispositivo e sono separate per provider e account. Se configurato, OTX riceve durante l'analisi soltanto URL completi, domini nativi del mittente, IP pubblici e hash SHA-256 presenti nell'email; il contenuto del messaggio e il file `.eml` non vengono inviati.
